...
def build_scoring_prompt(
    ...
    main_target = object_metadata.get("object") if object_metadata else object_name
    relation_context = object_metadata.get("relation_context") if object_metadata else ""
    reference_object = object_metadata.get("reference_object") if object_metadata else ""
    return f"""
You are scoring bounding-box candidates for Omni3D-Bench 3D object extraction.

Question context: {question}
Target object phrase: {object_name}
Main target: {main_target or object_name}
Relation context: {relation_context or 'none'}
Reference object: {reference_object or 'none'}
...
def _choose_candidate_by_scoring_vlm(
    ...
    prompt = build_scoring_prompt(question, object_name, candidates, object_metadata=object_metadata)
